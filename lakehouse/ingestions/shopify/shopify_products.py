...
log = get_logger("shopify_products")
...
def transform(df):
    ...
    return dedup(exploded, col("variant_updated_at").desc_nulls_last(), col("_ingested_at").desc())


def main():
    catalog, run_mode, source_date = parse_ingest_args("Shopify Products ingest")
    paths = build_paths(catalog, "products", source_date)
    ...
    if run_mode == "streaming":
        run_streaming(spark, paths, SHOPIFY_PRODUCTS_SCHEMA, transform, log, cluster_cols=["product_created_at", "product_updated_at"])
    else:
        run_batch(spark, paths, SHOPIFY_PRODUCTS_SCHEMA, transform, log, cluster_cols=["product_created_at", "product_updated_at"])
